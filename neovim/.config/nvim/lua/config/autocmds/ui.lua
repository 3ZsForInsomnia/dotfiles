-- UI and visual related autocmds
local v = vim
local a = v.api
local h = a.nvim_set_hl
local au = a.nvim_create_autocmd
local ag = a.nvim_create_augroup

local my_ns = a.nvim_create_namespace("my_autocmds")

-- Mode specific MsgArea, CursorLineNr and LineNr formatting
au("ModeChanged", {
  callback = function()
    local mode = v.api.nvim_get_mode().mode
    local colorsForCursorLine = {
      ["i"] = "#00ff00",
      ["n"] = "#00ffff",
      ["c"] = "#e0af68",
      ["v"] = "#c678dd",
      ["V"] = "#c678dd",
      [""] = "#c678dd",
    }

    h(0, "CursorLineNr", {
      foreground = colorsForCursorLine[mode] or "#666666",
      bold = true,
    })

    if mode == "c" then
      h(0, "MsgArea", {
        foreground = colorsForCursorLine[mode],
      })
      h(0, "LineNr", {
        foreground = colorsForCursorLine[mode],
      })
    else
      h(0, "MsgArea", {
        foreground = "#cccccc",
      })
      h(0, "LineNr", {
        foreground = "#666666",
      })
    end
  end,
})

-- Enhanced search count display with virtual text
_G.extended_search_count = function()
  if vim.v.hlsearch == 0 then
    -- Clear existing extmarks if highlighting is off
    local bufnr = a.nvim_get_current_buf()
    a.nvim_buf_clear_namespace(bufnr, my_ns, 0, -1)
    return
  end

  local shortmess_str = vim.o.shortmess

  if not shortmess_str:find("S") then
    return
  end

  local search_count_options = { maxcount = 100000, timeout = 500 }
  local sinfo = vim.fn.searchcount(search_count_options)

  local search_stat = sinfo.incomplete > 0 and "[?/?]"
    or sinfo.total > 0 and ("[%s/%s]"):format(sinfo.current, sinfo.total)
    or nil

  if search_stat ~= nil then
    local bufnr = a.nvim_get_current_buf()
    local line = a.nvim_win_get_cursor(0)[1] - 1

    a.nvim_buf_clear_namespace(bufnr, my_ns, 0, -1)

    a.nvim_buf_set_extmark(bufnr, my_ns, line, -1, {
      virt_text = { { search_stat, "Search" } },
      virt_text_pos = "eol",
    })
  end
end

-- Search count autocmds (converted from VimScript)
local search_group = ag("UpdateSearchCount", { clear = true })
au("CmdlineEnter", {
  group = search_group,
  pattern = { "/", "\\?" },
  callback = function()
    extended_search_count()
  end,
})

au("CmdlineLeave", {
  group = search_group,
  pattern = { "/", "\\?" },
  callback = function()
    extended_search_count()
  end,
})

au({ "CursorMoved", "CursorMovedI" }, {
  group = search_group,
  pattern = "*",
  callback = function()
    extended_search_count()
  end,
})
